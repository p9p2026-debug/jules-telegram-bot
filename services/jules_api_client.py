...
class JulesApiClient:
    ...
    async def approve_plan(
        ...
        """
        Approves the plan generated by Jules for a session.
        Endpoint: POST /v1alpha/{session_name}:approvePlan
        """
        clean_name = session_name if session_name.startswith("sessions/") else f"sessions/{session_name}"
        return await asyncio.to_thread(cls._execute_request, f"{clean_name}:approvePlan", "POST", {}, api_key)
